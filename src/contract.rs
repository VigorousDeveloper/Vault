...
pub fn execute_deposit(
    ...
    let user_address = deps.api.addr_validate(&msg.sender)?;
    ...
    if user_info == None {
        ...
        current_user_info.amount += msg.amount;
        store_users(deps.storage, &user_address, current_user_info)?;
    }
    current_state.total_money += msg.amount;
...
pub fn query_top_users(
    deps:Deps
) -> StdResult<Vec<UserInfo>>{
    let users = read_users(deps.storage)?;
    let mut user_info_group = vec![];
    for user in users.iter(){
        let user_info = read_user_info(deps.storage, &deps.api.addr_validate(&String::from(user))?).unwrap();
...
mod tests {
    ...
    fn deposit_and_withdraw() {
        ...
        let msg = ExecuteMsg::Deposit(cw20::Cw20ReceiveMsg {
            sender: "sender0".to_string(),
        ...
        assert_eq!(top_users,[UserInfo{address:"sender3".to_string(),amount:Uint128::new(20)}
        ,UserInfo{address:"sender1".to_string(),amount:Uint128::new(15)}]);
    }
}
